...
static PNIO_UINT8  ResponseDataAvailable = PNIO_FALSE;

#define RSP_MAXBUFSIZE  ( sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_GET_MODE ) + 20 )



PNIO_UINT8   PE_RspBuffer[ RSP_MAXBUFSIZE ];  /* buffer for PE response */
PNIO_UINT32  PE_RspLen;                       /* length of PE response (must be <= RSP_MAXBUFSIZE) */
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        if( ErrCode == PE_RET_OK )
        {
            ...
            PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_START_PAUSE );
        }
        else
        {
            PE_RspLen = PE_SetErrRsp( pRspBuf, ErrCode );
        }
    /*     END    user code */
    /* *************************************************************** */
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        if( ErrCode == PE_RET_OK )
        {
            ...
            PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_END_PAUSE );
            return( PNIO_OK );
        }
        else
        {
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        if( ErrCode == PE_RET_OK )
        {
			...
			#endif
            PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_QUERY_MODES );
        }
        else
        {
            PE_RspLen = PE_SetErrRsp( pRspBuf, ErrCode );
    ...
    /*     BEGIN  user code */
        ...
        if( ErrCode == PE_RET_OK )
        {
            ...
            if( CurrentPE_Mode == PE_MODE_READY_TO_OPERATE )  			/* current state is OPERATE */
            {
                ...
                Val32 = 0;												/* 0 sec, is already operating */
                SetVal32_Htonl( &( pRspGetM->TimeToOperate ), &Val32 );
            ...
            PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_GET_MODE );
        }
        else
        {
            PE_RspLen = PE_SetErrRsp( pRspBuf, ErrCode );
    ...
    /*     BEGIN  user code */
        ...
        if( ErrCode == PE_RET_OK )
        {
            if( CurrentPE_Mode == PE_MODE_READY_TO_OPERATE )
            {
                ...
                PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_PEM_STATUS );
            }
            else
            {
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        if ( ErrCode == PE_RET_OK )
        {
            ...
            PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_IDENTIFY );
        }
        else
        {
            PE_RspLen = PE_SetErrRsp( pRspBuf, ErrCode );
    ...
    )
{
    ...
    /*     BEGIN  user code */
		...
		{
			ErrCode = PE_RET_INVALID_DATA_STRUCT_IDENT_RQ;
		}
		else
		{
        ...
        if( ErrCode == PE_RET_OK )
        {
            ...
            PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_GET_MEAS_VALUES );
        }
        else
        {
            PE_RspLen = PE_SetErrRsp( pRspBuf, ErrCode );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        if( ErrCode == PE_RET_OK )
        {
			...
			PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_GET_MEAS_LIST );
        }
        else
        {
        	PE_RspLen = PE_SetErrRsp( pRspBuf, ErrCode );
    ...
    /*     BEGIN  user code */
        ...
        if ( ErrCode == PE_RET_OK )
        {
            if ( CurrentPE_Mode == PE_MODE_READY_TO_OPERATE )
            {
                ...
                PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_PEM_CTTO_STATUS );
            }
            else
            {
                pRspPemStat->PeModeIdSrc   = PE_MODE_PAUSE1;                  		/* PE mode ID source */
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        if ( ErrCode == PE_RET_OK )
        {
            ...
            PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_START_PAUSE_WITH_TIME_RESP );
        }
        else
        {
            PE_RspLen = PE_SetErrRsp ( pRspBuf, ErrCode );
    ...
    /*     BEGIN  user code */
        ...
        if (ErrCode == PE_RET_OK)
        {
          /* used version V1.1 */
            ...
            PE_RspLen = sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_VERSION );
        }
        else
        {
            PE_RspLen = PE_SetErrRsp ( pRspBuf, ErrCode );
    ...
    )
{
    ...
    ResponseDataAvailable = PNIO_TRUE;
   ...
   /* *********** check block header ********** */
    ...
    OsMemCpy( ( PNIO_VOID* )&PE_RspBuffer[ 0 ], ( PNIO_VOID* ) pSumReqHdr, sizeof( PE_SUM_REQ_HDR ) );
    ...
    if( PE_RET_OK == pe_process_resp.pe_response )
    {
        ...
        if( Status == PNIO_OK )
        {
          ( pe_process_resp.pe_method_ptr ) ( pBuffer , &PE_RspBuffer[ 0 ] );
        }
        else
        {
            pe_process_resp.pe_response = PE_RET_INVALID_BLOCK_HEADER;
            PE_RspLen = PE_SetErrRsp( &PE_RspBuffer[ 0 ], pe_process_resp.pe_response );
        }
    }
    ...
    )
{
    ...
    /* ----------------------------------------------------------------------- */
    if( ResponseDataAvailable == PNIO_FALSE )
    {
        pPnioState->ErrCode   = 0xde;  /* IODReadRes with ErrorDecode = PNIORW */
        pPnioState->ErrDecode = 0x80;  /* PNIORW */
    ...
    { /* ** reset flag "response data available", so a new write request must follow.. ** */
        ResponseDataAvailable  = PNIO_FALSE;
    }

    /* **** copy the provided response data to the specified destination buffer *** */
    OsMemCpy( pBuffer, &PE_RspBuffer[ 0 ], PE_RspLen );

    /* **** limit to buffer size and copy receive buffer **** */
    if( PE_RspLen < *pBufLen )
        *pBufLen = PE_RspLen;

    OsMemCpy ( pBuffer, &PE_RspBuffer[ 0 ], *pBufLen );
