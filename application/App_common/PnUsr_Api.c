...
#include "version_dk.h"
    ...
    PNIO_ANNOTATION*  UsrIod_BuildDeviceAnnotation (void)
    {
        ...
        OsMemCpy (DevAnnotation.OrderId,
                DEV_ANNOTATION_ORDER_ID,
                OsStrLen (DEV_ANNOTATION_ORDER_ID));

        OsMemCpy (DevAnnotation.SerialNumber,
        		IM0_SERIAL_NUMBER,
                OsStrLen (IM0_SERIAL_NUMBER));

		#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200P )
		if(0x0100 == (REG32( U_SCRB__ID_REG ) & 0x0000FF00))
		{
			DevAnnotation.HwRevision = DEV_ANNOTATION_HW_REVISION_ERTEC200P;
		}
		else
		{
			DevAnnotation.HwRevision = DEV_ANNOTATION_HW_REVISION_ERTEC200P2;
		}
		#else
		DevAnnotation.HwRevision = DEV_ANNOTATION_HW_REVISION_ERTEC200P;  // defined as big endian in PNIO spec.
		#endif
...
#define SNMP_SYSTEM_DESCRIPTION_ACCORDING_TO_SPH
            ...
            SnmpPar.pSysDescr   = SNMP_SYSTEM_DESCRIPTION;
            SnmpPar.SysDescrLen = OsStrLen (SNMP_SYSTEM_DESCRIPTION);
#else
            SysDescrBuff_size = 0;
            OsAllocF ((void**)&SysDescrBuff, 0xff);
            SnmpPar.pSysDescr = (PNIO_INT8*)SysDescrBuff;
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) DEVKIT_VENDOR);
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) ", ");
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) DEVKIT_PRODUCTFAMILY);
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) ", ");
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) DEVKIT_PRODUCTNAME);
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) ", ");
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) DEVKIT_ORDERNUMBER);
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) ", ");
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) "%04d", DEVKIT_HW_REVISION);
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) ", ");
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) "%c%02d.%02d.%02d.%02d", DEVKIT_VERSION_PREFIX,
            																										DEVKIT_VERSION_HH,
																													DEVKIT_VERSION_H,
																													DEVKIT_VERSION_L,
																													DEVKIT_VERSION_LL);
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) ", ");
            SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size],  (PNIO_INT8*) DEVKIT_PRODUCTSERIALNUMBER);
            ...
            SnmpPar.pIfDescr    = SNMP_INTERFACE_DESCRIPTION;
            SnmpPar.IfDescrLen  = OsStrLen (SNMP_INTERFACE_DESCRIPTION);

            SnmpPar.pPortName1   = SNMP_PORT1_NAME;
            SnmpPar.PortNameLen1 = OsStrLen (SNMP_PORT1_NAME);

            #if (IOD_PDEV_NUMOF_PORTS >= 2)
                SnmpPar.pPortName2   = SNMP_PORT2_NAME;
                SnmpPar.PortNameLen2 = OsStrLen (SNMP_PORT2_NAME);
            #endif

            #if (IOD_PDEV_NUMOF_PORTS >= 3)
                SnmpPar.pPortName3   = SNMP_PORT3_NAME;
                SnmpPar.PortNameLen3 = OsStrLen (SNMP_PORT3_NAME);
            #endif

            #if (IOD_PDEV_NUMOF_PORTS >= 4)
...
// ***==================================================================================***
	...
	)
    {
        ...
        switch (RecordIndex)
        {
            ...
            case 0x80a0:
                    Status = PROFIenergy_ResponseHandler (pAddr, pBufLen, pTmpBuf, pPnioState);
                    break;

            #if (INCLUDE_IM0_4_HANDLING == 0)      // only if IM is not handled inside the PN stack..
			...
			case 0xaff0: // **** IM 0 *****
					...
					break;
			case 0xaff1: // **** IM 1 *****
					PNIO_printf ( (PNIO_INT8*) "##READ IM1 Data, Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
								 Api, slot_nr, subslot_nr, *pBufLen, SequenceNum);
					Status = Im1_read_Handler (Api, pAddr, pBufLen, pTmpBuf, pPnioState);
					break;
			case 0xaff2: // **** IM 2 *****
					PNIO_printf ( (PNIO_INT8*) "##READ IM2 Data, Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
								 Api, slot_nr, subslot_nr, *pBufLen, SequenceNum);
					Status = Im2_read_Handler (Api, pAddr, pBufLen, pTmpBuf, pPnioState);
					break;
			case 0xaff3: // **** IM 3 *****
					PNIO_printf ( (PNIO_INT8*) "##READ IM3 Data, Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
								 Api, slot_nr, subslot_nr, *pBufLen, SequenceNum);
					Status = Im3_read_Handler (Api, pAddr, pBufLen, pTmpBuf, pPnioState);
					break;
			case 0xaff4: // **** IM 4 *****
					PNIO_printf ( (PNIO_INT8*) "##READ IM4 Data, Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
								 Api, slot_nr, subslot_nr, *pBufLen, SequenceNum);
					Status = Im4_read_Handler (Api, pAddr, pBufLen, pTmpBuf, pPnioState);
					break;
			#endif // (INCLUDE_IM0_4_HANDLING == 0)
	        // *----------------------------------------------*
	...
	)
    {
        ...
        {
            case 0x80a0:
                    Status = PROFIenergy_RequestHandler (pAddr, pBufLen, pBuffer, pPnioState);
                    break;
