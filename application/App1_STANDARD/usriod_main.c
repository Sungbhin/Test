/*---------------------------------------------------------------------------*/
/*  Copyright (C) 2016 Siemens Aktiengesellschaft. All Rights Reserved.      */
...
#include "usrapp_cfg.h"   // example application spec. configuration settings
        ...
        // * --------------------------------------------
		   {0,                          // PNIO_UINT32     Api;                 // api number
		    ...
		    0x8000,                          // PNIO_UINT32     Subslot;             // subslot number (1..0x7fff)
		   	   {0x2a,                   // PNIO_UINT16     VendorId;            // VendorIDHigh, VendorIDLow
			   DEV_ANNOTATION_ORDER_ID,// PNIO_UINT8      OrderId [20];        // Order_ID, visible, must be 20 bytes here (fill with blanks)
			   IM0_SERIAL_NUMBER,     // PNIO_UINT8      SerNum  [16];        // IM_Serial_Number, visible string, must be 16 bytes here (fill with blanks)
			   DEVKIT_HW_REVISION,     // PNIO_UINT16     HwRevision;          // IM_Hardware_Revision
			   {DEVKIT_VERSION_PREFIX,  // PNIO_UINT8      SwRevPrefix;         // software revision prefix
			   DEVKIT_VERSION_HH,      // PNIO_UINT8      SwRevFuncEnhanc;     // IM_SW_Revision_Funct_Enhancement
			   DEVKIT_VERSION_H ,      // PNIO_UINT8      SwRevBugFix;         // IM_SW_Revision_Bug_Fix
 ...
 // *----------------------------------------------------------------*
...

    {
        ...
        char* addr_0 = (volatile char *)0x30000000;
        volatile char a = *addr_0;
	    ...
	    while (exit == PNIO_FALSE)
	    {
		    ...
		    switch (PressedKey)
		    {
			    ...
			    case 's':
				        ...
				        break;
			    case 'Z':
			    	REG32(U_EMC_REG__ASYNC_BANK1)&= 0x0;
			    	REG32(U_EMC_REG__ASYNC_BANK1) |= 0x0834450D;
			    	REG32(U_EMC_REG__ASYNC_BANK0)&= 0x0;
			    	REG32(U_EMC_REG__ASYNC_BANK0) |= 0x084450D;
			    	PNIO_ConsolePrintf("Async_BANK 0 : %08x...\n", REG32(U_EMC_REG__ASYNC_BANK0));
			    	PNIO_ConsolePrintf("Async_BANK 1 : %08x...\n", REG32(U_EMC_REG__ASYNC_BANK1));
			    	break;
			    case 'h':
			    	while(1)
			    	{
			    	...
			    	PNIO_ConsolePrintf("0x3000_0000 : %d\n", a);
			    	break;
			    case 'H':
			    	while(1)
			    	{
